...
public class link extends ServiceBase {
	...
	public link() {
		lm = (LinkManager) SpringContext.getContext().getBean("linkManager");
		tm = (TemplateManager) SpringContext.getContext().getBean("templateManager");
	}

	/**
	 * method: get description: get link to crawl parameter :action, list -
	 * shows the current list, get - get a link, this will remove the link as
	 * well redirect - returns an html page that automatically redirect to next
	 * link. count, how many links to return, if action is list, default to 20,
	 * if action is get, default to 1. -1 will return all.
	 * 
	 * 
	 */
	@Override
	...
	/**
	 * method: post description: add link(s) to crawl parameters: data,
	 * containing json array of links. response: how many requests are added,
	 * some requests might be in system already. An example response is
	 * "{'result':'2'}".
	 */
	@Override
