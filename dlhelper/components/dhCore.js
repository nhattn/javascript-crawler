...
Core.prototype.registerMenu=function(menupopup,menutype) {
	...
	if(menutype==Components.interfaces.dhICore.MENU_TYPE_DOWNLOAD) {
		...
		this.regMenus.push({
			// menupopup:
			// menupopup.QueryInterface(Components.interfaces.nsISupportsWeakReference).GetWeakReference(),
			menupopup: menupopup,
			menutype:menutype,
...
Core.prototype.observe=function(subject, topic , data) {
	...
	switch(topic) {
		...
		case "http-on-examine-response":
		    var request=subject.QueryInterface(Components.interfaces.nsIRequest);
			// dump("[Core] observe/http-on-examine-response
			// "+request.name+"\n");
		    if(this.listMgr.checkCurrentURL(request.name)) {
				try {
				    var channel=subject.QueryInterface(Components.interfaces.nsIHttpChannel);
...
Core.prototype.handleEvent=function(event) {
	// dump("[Core]
	// handleEvent("+event.type+"/"+event.eventPhase+"/"+event.target+")\n");
...
Core.prototype.addEntryForDocument=function(entry,document,window) {
	// dump('addentry 1:'+document.body.innerHTML);
	try {
...
Core.prototype.updateMenus=function(document,window) {
	// dump("[Core]
	// updateMenus("+(document?document.URL:null)+","+(window?window.content.document.URL:"window")+")\n");
	...
	try {
		...
		for(var i in this.regMenus) {
			...
			if(window==null || window==menu.window) {
				if(menu.menupopup) {
					// var
					// menupopup=menu.menupopup.QueryReferent(Components.interfaces.nsIDOMElement);
...
// dump('hello \n');
// try{
// setInterval('commandListener.handleEvent({type:"command"})', 1000);
// }catch(e){
// dump(e);
// }
...
// try{
	...
	if(true) {
		...
		// dump(tooltipText);
					...
					var w = entry.get("window-document",Components.interfaces.nsISupports);
			 					
				 
					// var w = window;
					var bd = w.document.body;
					var sc =w.document.createElement("button");
					var _self = this;
					sc.addEventListener('click', function(){_self.handleCommand(entry, 0);dump(2);}, false);
					sc.textContent='dkdxxxxxxxxxxxxkd';
	...
	for(var i=0;i<this.entries.length;i++) {
		...
		if(Util.getPropsString(entry,"entry-type")=="expirable") {
			...
			if(timeNow>created+expireTimeout*1000) {
				// dump("[Core] cleanupExpiredEntry(): removed
				// "+Util.getPropsString(entry,"media-url")+"\n");
				tbd.push(entry);
			}
		}
	}
		...
		WindowMonitor.prototype={
			handleEvent: function(event) {
				switch(event.type) {
					case "pageshow":
						try {
							// dump("pageshow - "+event.target.URL+"
							// "+event.target+"\n");
...
// dump('handle key:'+key+'\n');
	try {
		...
		if(processor==null) {
			...
			// dump("[Core] handleCommand(entry,"+key+") processor:
			// "+data.processor+"\n");
