...
public class Utils {
	...
	public static String getUniqueBookId() {
		...
		String l = StringUtils.leftPad(Integer.toString(Math
				.abs(rand.nextInt())), 15, '0');
		buf.append(l);
		return buf.toString();
	}
		...
		try {
			if (values.length % 2 != 0)
				throw new IOException(
						"Wrong number of values for method, not even.");
			...
			for (Object o : set) {
				if (o instanceof String) {
					...
					if (v != null && v instanceof String) {
						...
						value = new String(value.getBytes(Config.Encoding),
								Config.Encoding_DB);
						try {
