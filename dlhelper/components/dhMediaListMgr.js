...
const
NS_DH_MEDIA_LIST_MGR_CID = Components.ID("{55f8f6d3-9ac7-4046-b1bb-7c732e27d2d6}");
const
NS_DH_MEDIA_LIST_MGR_PROG_ID = "@downloadhelper.net/media-list-manager";
const
DHNS = "http://downloadhelper.net/1.0#";
...
function DhMediaListMgr() {
	try {
		...
		this.storageFile = Components.classes["@mozilla.org/file/directory_service;1"].getService(Components.interfaces.nsIProperties).get("ProfD", Components.interfaces.nsIFile);
...
DhMediaListMgr.prototype.addToList = function(list, url, type, pageurl, filename, referer) {
	//dump(url + '  ' + type + '  ' + pageurl);
	// dump("[DhMediaListMgr]
	// addToList("+list+","+url+","+type+","+pageurl+","+filename+","+referer+")\n");
	...
	try {
		...
		for ( var i = 0; i < entries.length; i++) {
			if (Util.getPropertyValueRS(this.dataSource, entries[i], DHNS + "url") == url) {
				// dump("[DhMediaListMgr] !!! addToList(): removing previous
				// entry\n");
...
DhMediaListMgr.prototype.removeCurrentURL = function(item) {
	for ( var i = 0; i < this.currentURLs.length; i++) {
		...
		if (entry.url == item) {
			this.currentURLs.splice(i, 1);
			// dump("[DhMediaListMgr] removeCurrentURL("+entry.url+")
			// removed\n");
			return;
		}
	}
...
DhMediaListMgr.prototype.checkCurrentURL = function(item) {
	...
	// dump("[DhMediaListMgr] checkCurrentURL("+item+") (out of
	// "+this.currentURLs.length+") not found\n");
	return false;
}
...
DhMediaListMgr.prototype.QueryInterface = function(iid) {
	...
	if (iid.equals(Components.interfaces.nsITimerCallback) == false && iid.equals(Components.interfaces.nsIObserver) == false && iid.equals(Components.interfaces.dhIMediaListMgr) == false
...
var vDhMediaListMgrModule = {
	...
	/*
	 * RegisterSelf is called at registration time (component installation or
	 * the only-until-release startup autoregistration) and is responsible for
	 * notifying the component manager of all components implemented in this
	 * module. The fileSpec, location and type parameters are mostly opaque, and
	 * should be passed on to the registerComponent call unmolested.
	 */
	...
	compMgr.registerFactoryLocation(NS_DH_MEDIA_LIST_MGR_CID, "DhMediaListMgr", NS_DH_MEDIA_LIST_MGR_PROG_ID, fileSpec, location, type);
...
vDhMediaListMgrFactory : {
	...
	if (Util == null)
		Util = Components.classes["@downloadhelper.net/util-service;1"].getService(Components.interfaces.dhIUtilService);
...
/*
 * The canUnload method signals that the component is about to be unloaded. C++
 * components can return false to indicate that they don't wish to be unloaded,
 * but the return value from JS components' canUnload is ignored: mark-and-sweep
 * will keep everything around until it's no longer in use, making unconditional
 * ``unload'' safe.
 * 
 * You still need to provide a (likely useless) canUnload method, though: it's
 * part of the nsIModule interface contract, and the JS loader _will_ call it.
 */
